...
def movePipe(xy, d):
    ...
    if d == 1: # 가로
        if xy[1] + 1 < N:
            if not home[xy[0]][xy[1]+1]: #가로로 갈 수 있다.
                movePipe([xy[0], xy[1]+1], 1)
            if xy[0] + 1 < N and not home[xy[0]][xy[1]+1] and not home[xy[0]+1][xy[1]] and not home[xy[0]+1][xy[1]+1]: # 대각선으로 갈 수 있다.
                movePipe([xy[0]+1, xy[1]+1], 3)
    elif d == 2: # 세로
        if xy[0] + 1 < N:
            if not home[xy[0]+1][xy[1]]:
                movePipe([xy[0]+1, xy[1]], 2)
            if xy[1] + 1 < N and not home[xy[0]][xy[1]+1] and not home[xy[0]+1][xy[1]] and not home[xy[0]+1][xy[1]+1]:
                movePipe([xy[0]+1, xy[1]+1],3)
    elif d == 3: # 대각선
        if xy[1] + 1 < N:
            if not home[xy[0]][xy[1]+1]: #가로로 갈 수 있다.
                movePipe([xy[0], xy[1]+1], 1)
            if xy[0] + 1 < N and not home[xy[0]][xy[1]+1] and not home[xy[0]+1][xy[1]] and not home[xy[0]+1][xy[1]+1]:
                movePipe([xy[0]+1, xy[1]+1],3)
        if xy[0] + 1 < N:
            if not home[xy[0]+1][xy[1]]:
                movePipe([xy[0]+1, xy[1]], 2)
